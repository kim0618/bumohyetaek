import type { ContentItem } from '@/types/content';

const article: ContentItem = {
  slug: 'disability-registration',
  title: '장애인 등록 방법과 복지 혜택 총정리',
  seoTitle: '장애인 등록 신청 방법 - 혜택과 복지카드 발급',
  seoDescription: '장애 판정을 받았다면 장애인 등록으로 다양한 복지 혜택을 받을 수 있습니다. 등록 신청 절차, 장애 정도 심사, 복지카드 발급 방법과 주요 혜택을 정리했습니다.',
  category: 'pension-welfare',
  template: 'policy',
  targetUser: 'both',

  createdAt: '2026-03-30',
  updatedAt: '2026-04-18',
  reviewStatus: 'current',

  summary:
    '장애인 등록을 하면 의료비 감면, 교통 할인, 세금 감면 등 다양한 혜택을 받을 수 있습니다. 신청 절차부터 장애 정도 심사, 복지카드 발급까지 정리했습니다.',
  heroDescription:
    '질병이나 사고로 장애를 갖게 된 경우 장애인 등록을 통해 복지 혜택을 받을 수 있습니다. 노인성 질환(파킨슨병·뇌졸중 등)으로도 장애 등록이 가능합니다.',

  keyPoints: [
    '주민센터 신청 → 국민연금공단 심사 → 장애 정도 결정',
    '장애 정도가 심한 경우(기존 1~3급) 의료급여 1종 자동 적용',
    '복지카드로 교통·문화시설 등 다양한 할인 혜택 이용',
  ],

  tags: ['장애인등록', '복지카드', '장애인혜택', '장애등급', '노인복지'],
  hubKey: ['government-benefits'],
  order: 14,
  isFeatured: false,

  officialSources: [
    {
      name: '보건복지부',
      url: 'https://www.mohw.go.kr',
      note: '장애인 등록 안내',
    },
    {
      name: '복지로',
      url: 'https://www.bokjiro.go.kr',
      note: '장애인 복지서비스 신청',
    },
    {
      name: '국민연금공단',
      url: 'https://www.nps.or.kr',
      note: '장애 정도 심사',
    },
  ],

  sections: [
    {
      type: 'text',
      heading: '노인은 장애인 등록이 안 된다고 생각하기 쉽지만',
      body: '나이가 들면서 생긴 장애는 등록 대상이 아니라고 오해하는 경우가 많습니다. 실제로는 뇌졸중 후유증, 파킨슨병, 당뇨 합병증 등 노인성 질환으로 인한 장애도 등록이 가능합니다. 신체적·정신적 장애로 일상생활이나 사회생활에 제한이 있으면 누구나 대상입니다. 만 65세 이상은 장기요양과 장애 혜택을 동시에 받을 수 있어 이중으로 지원받는 셈입니다. [[long-term-care-grade|장기요양 등급]]도 함께 확인하세요.',
    },
    {
      type: 'info',
      heading: '장애 정도 구분',
      body: '2019년부터 장애 등급(1~6급)이 폐지되고 "장애 정도가 심한 장애인"과 "장애 정도가 심하지 않은 장애인"으로 단순화되었습니다. 정도가 심한 경우 더 많은 혜택을 받습니다.',
    },
    {
      type: 'list',
      heading: '신청 시 필요한 서류',
      items: [
        '신분증',
        '장애 진단서(지정 의료기관 발급)',
        '진료기록지·검사결과지',
        '사진 1장(3×4cm)',
      ],
    },
    {
      type: 'numbered-list',
      heading: '등록 신청 절차',
      items: [
        '주민센터 방문: 장애인 등록 신청서 제출, 신분증 지참',
        '진단 의뢰서 발급: 주민센터에서 발급받아 해당 병원 방문',
        '병원 진단: 장애 유형별 전문의에게 장애 진단서 발급 (비용 본인 부담)',
        '국민연금공단 심사: 진단서 제출 후 장애 정도 심사 (약 30일 소요)',
        '결과 통보: 장애 정도 결정 후 복지카드 신청 가능',
      ],
    },
    {
      type: 'list',
      heading: '주요 복지 혜택',
      items: [
        '의료비: 장애 정도 심한 경우 의료급여 1종 적용, 건강보험료 경감',
        '교통: 철도·항공 50% 할인, 고속도로 통행료 50% 할인, 지하철 무료',
        '세금: 자동차 취득세·등록세 면제(장애 정도 심한 경우), 소득세·상속세 공제',
        '통신: 이동통신 요금 할인(월 최대 26,000원)',
        '문화: 국공립 박물관·공원·체육시설 무료 또는 할인',
        '주거: 공공임대주택 우선 공급',
      ],
    },
    {
      type: 'tip',
      heading: '장애인 활동지원 서비스',
      body: '장애 정도가 심한 만 6세~65세 미만 장애인은 "장애인 활동지원" 서비스를 신청할 수 있습니다. 만 65세 이상이 되면 활동지원 대신 [[elderly-care-service|노인돌봄서비스]] 또는 장기요양서비스로 전환됩니다. 전환 시 서비스 차이를 미리 확인하세요.',
    },
    {
      type: 'summary',
      heading: '핵심 요약',
      items: [
        '주민센터 신청 → 진단 → 국민연금공단 심사 순서로 진행',
        '노인성 질환(뇌졸중·파킨슨병)도 장애 등록 가능',
        '교통·의료·세금 감면 등 혜택이 많으므로 조건 해당 시 꼭 신청',
      ],
    },
  ],

  faq: [
    {
      question: '뇌졸중이나 파킨슨병으로도 장애인 등록이 가능한가요?',
      answer:
        '네, 가능합니다. 뇌졸중 후유증·파킨슨병·치매 등 노인성 질환으로 인해 일상생활이나 사회생활에 제한이 생기면 장애인 등록을 신청할 수 있습니다. 거주지 주민센터에 방문해 상담을 받아보세요.',
    },
    {
      question: '장애인 등록 심사에는 얼마나 걸리나요?',
      answer:
        '주민센터에 신청한 후 병원에서 진단서를 받아 제출하면 국민연금공단이 장애 정도를 심사합니다. 통상 30일 정도 소요되며, 복잡한 경우 더 길어질 수 있습니다. 처음 신청 시 담당 공무원에게 예상 기간을 확인하세요.',
    },
    {
      question: '장애인 등록을 하면 기초연금이나 의료급여도 자동으로 받나요?',
      answer:
        '자동으로 주어지지는 않습니다. 장애 정도가 심한 장애인은 의료급여 1종 대상이 되지만, 기초생활수급자로 선정되어야 합니다. 기초연금은 만 65세 이상 소득 하위 70% 조건을 별도로 충족해야 합니다. 각 혜택을 별도로 신청하세요.',
    },
    {
      question: '장애 정도 심사 결과에 불복할 수 있나요?',
      answer:
        '결과에 이의가 있으면 통보를 받은 날부터 90일 이내에 국민연금공단에 이의 신청을 할 수 있습니다. 이의 신청 시 추가 검사를 받게 되며, 재심사 결과에도 불복하면 행정심판이나 행정소송을 제기할 수 있습니다. 이의 신청 비용은 무료입니다.',
    },
    {
      question: '장애인 복지카드는 어디서 발급받나요?',
      answer:
        '장애 등록이 완료되면 주민센터에서 장애인 복지카드를 신청할 수 있습니다. 발급까지 약 2~3주 소요되며, 복지카드가 있어야 교통·문화시설 할인 등 각종 혜택을 현장에서 바로 이용할 수 있습니다. 신분증과 사진 1장을 준비하세요.',
    },
  ],

  cautionNote:
    '장애 판정 기준과 복지 혜택은 변경될 수 있습니다. 자세한 사항은 거주지 주민센터 또는 보건복지상담센터(☎ 129)에 문의하세요.',

  relatedSlugs: ['basic-livelihood-benefit', 'senior-discount-guide', 'medical-aid-system'],
};

export default article;
