...
- 사용자 명시 없으면 **10~12개 항목** 점검 (2026-06-22 확정, 아래 "가동 주기"와 동일 기준. 종전 "5~7개"는 항목이 적던 시절 값이라 한 바퀴가 너무 길어져 stale이 쌓인다. jptcalc-refresh에서 이 구값 때문에 배치 미완주가 실제 발생)
- **마지막 확인일 오래된 순** 정렬 후 픽업
- **점검한 항목은 그 항목의 "마지막 확인" 필드를 반드시 갱신할 것.** 변경 이력에만 적고 필드를 두면 다음 회전에서 같은 항목이 또 stale로 잡혀 중복 점검된다
