...
## 🔍 Cross-Reference 사실 검증 (필수, 작성 전)

**근거**: 2026-05-04 ltci-short-term-care 작성 시 "1회 최대 15일 / 인지지원등급 이용 불가"로 잘못 작성. care-type-selection 글에 이미 정확한 정보("1~5등급 및 인지지원등급 모두 이용 가능")가 있었으나 미참조.

**작성 전 절차**:

1. **policy-data-points.md 검색**: 새 글의 핵심 수치·등급·한도일수·대상 기준이 이미 마스터 리스트에 등록되어 있는지 확인. 등록된 값이 있으면 **반드시 그 값을 사용**.

2. **유사 주제 verified 글 cross-reference**: 새 글이 다루는 제도·서비스를 언급하는 기존 글을 grep으로 찾고, 핵심 사실(한도일수·이용 가능 등급·본인부담률 등)을 어떻게 서술했는지 확인.
   ```bash
   grep -rn "키워드" /home/tjd618/bumohyetaek/src/data/articles/ | grep -v 새글slug
   ```
   - 기존 글과 충돌하는 사실은 **반드시 공식 출처로 재확인**
   - 새 글이 정확하면 기존 글을 정정 (audit 회귀 검증 룰 적용)
   - 기존 글이 정확하면 새 글을 그 기준에 맞춰 작성

3. **WebSearch로 공식 정보 확인** (특히 다음 항목):
   - 정책 수치 (금액·요율·한도)
   - 대상 등급·연령·소득 기준
   - 신청 절차 일수·기관
   - 공식 사이트 도메인 (사설 도메인이 정부기관 명칭으로 등록된 경우 차단)

**위반 시 영향**: 잘못된 사실이 들어간 글은 게시 시 YMYL 신뢰도 직접 타격, 회귀 정정 작업 필요.
...
**"국가건강검진"(일반)과 "국가암검진"(6종) 혼용 금지.** 위/대장/간/유방/자궁경부/폐 6종 검진은 "국가암검진" 체계이며 아래 연령·주기를 정확히 써야 한다.

| 암 종류 | 대상 연령 | 주기 | 검사 |
|---------|-----------|------|------|
| 위암 | 만 40세~ | 2년마다 | 위내시경 또는 위장조영검사 |
| 대장암 | 만 50세~ | 매년 | 분변잠혈검사 → 양성 시 대장내시경 |
| 간암 | 만 40세~ 고위험군(간경변·B·C형 간염) | 6개월마다 | 간초음파 + 혈청 검사 |
| 유방암 | 만 40세~ 여성 | 2년마다 | 유방촬영술 |
| 자궁경부암 | 만 20세~ 여성 | 2년마다 | 자궁경부세포검사 |
| 폐암 | 만 54~74세 흡연력 30갑년 이상 | 2년마다 | 저선량 CT |

본인부담: 건강보험 가입자 10%, 의료급여 수급자·건강보험료 하위 50% 무료.

**의학 단정 표현 금지 (위반 시 YMYL 리스크)**
- "완치율이 크게 높아진다" → "치료 결과가 좋아지는 경향이 있다"
- "~만으로 개선됩니다" → "~해도 나아지는 경우가 많습니다"
- "반드시 ~하면 낫는다" → "~하는 것이 도움이 될 수 있습니다"
- 자가진단·자가치료 유도 표현 금지 ("약국에서 X약 사서 먹으면 된다" 류)
...
- body에 pipe table 금지
- 의학·금융 단정 표현 금지: "완치된다", "100% 낫는다", "~만으로 개선됩니다", "반드시 ~한다"
  → "~하는 경향이 있다", "~해도 나아지는 경우가 많다", "~하는 것이 좋습니다"로 완화
...
□ health-care: YMYL 단정 표현 grep 체크 ("완치", "100%", "만으로 개선", "반드시 낫")
□ health-care: 검진 수치 정확 (국가암검진 연령·주기, 국가건강검진과 혼용 금지)
□ em dash(-) grep 체크
...
절차:
1. `/home/tjd618/bumohyetaek/policy-data-points.md` 읽기
2. 새 글의 `sections`, `summary`, `keyPoints`, `seoDescription`에서 다음 패턴 검색:
   - 금액: `[0-9,]+원`, `[0-9]+만\s*원`
   - 비율: `[0-9.]+%`
   - 연령·기간: `만\s*[0-9]+세`, `[0-9]+년`
   - 정책 키워드: 기초연금, 국민연금, 장기요양, 건강보험료, 산정특례, 차상위, 중위소득, 연금저축, IRP, 문화누리, 에너지바우처 등
3. 매칭 시도:
   - **기존 항목과 일치** (예: "349,700원", "13만원" 등): 해당 항목의 "영향 글" 목록에 새 slug 추가
   - **신규 정책 수치 발견**: 사용자에게 "마스터 리스트에 신규 항목으로 등록할까요?" 묻기 → 승인 시 등록
4. 마스터 리스트 수정 시 변경 이력 섹션에 기록:
   ```
   ### YYYY-MM-DD - 신규 글 등록 (slug)
   - 영향 글 추가: [항목명] ← [slug]
   ```

### 5. 결과 요약
모든 작업 완료 후 추가한 파일 목록, 주요 섹션 구성, FAQ 수, 도입부 패턴, 빌드 결과, **마스터 리스트 등록 항목**을 요약해서 알려준다.
...
```bash
python3 -c "import json,datetime; logs=json.load(open('/home/tjd618/skill-log.json')); now=datetime.datetime.now(); logs.insert(0,{'date':now.strftime('%Y-%m-%d'),'time':now.strftime('%H:%M'),'project':'bumohyetaek','skill':'bumo-content'}); open('/home/tjd618/skill-log.json','w').write(json.dumps(logs,ensure_ascii=False,indent=2))"
```
